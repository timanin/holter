...
public class RetrieveItemResource {
	...
	public RetrieveItemResource() {
		...
		logger = LoggerFactory
				.getLogger("com.sergeytimanin.holter.resources.RetrieveItemResource");
	...
	public String sayHelloPlain(@QueryParam("service") String service,
			...
			@QueryParam("item") Optional<String> item,
			@QueryParam("debug") Optional<Boolean> debug) throws Exception {
		...
		if (!item.isPresent() || item.get().isEmpty()) {
			...
			jmxValue = jmxc.getJmxItem(service, object, attribute, item.get())
					.toString();
		}
